...
# ----------------------------------------------------------------------
class JetAgent(EvoAgent):

    def __init__(self, state_size, n_actions, architecture=JetNet):
        super(JetAgent, self).__init__(state_size, n_actions, architecture)
        self.reinitialize()
...
def training_loop(agent, environment, config, callbacks):
    ...
    agent_pool = [JetAgent(state_size, n_actions) for _ in range(config.pool_size)] 
...
def next_generation(agent_pool, environment, parent_indices=[], noise=0.01):
    ...
    for idx in parent_indices:
        ...
        new_agent.evaluate(environment, max_reward=1)
...
def noise_level(noise_range, generation, max_generations):
    # try noise decay
    return noise_range[0] - generation * (noise_range[0] - noise_range[1]) / (max_generations - 1)