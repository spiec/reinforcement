...
# ----------------------------------------------------------------------
class PongAgent(EvoAgent):

    # ----------------------------------------------------------------------
    def __init__(self, state_size, n_actions, architecture=PongNet):
        super(PongAgent, self).__init__(state_size, n_actions, architecture)
        self.reinitialize()
...
def training_loop(agent, environment, config, callbacks):
    ...
    agent_pool = [PongAgent(state_size, n_actions) for _ in range(config.pool_size)] 
